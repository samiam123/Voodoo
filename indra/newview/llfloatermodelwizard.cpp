	 ...
	 ,mPhysicsPreviewPanel(NULL)
{
	...
	getChild<LLButton>("choose_file_btn")->setCommitCallback(boost::bind(&LLFloaterModelWizard::setState, this, CHOOSE_FILE));
	getChild<LLButton>("optimize_btn")->setCommitCallback(boost::bind(&LLFloaterModelWizard::setState, this, OPTIMIZE));
	getChild<LLButton>("physics_btn")->setCommitCallback(boost::bind(&LLFloaterModelWizard::setState, this, PHYSICS));
	getChild<LLButton>("review_btn")->setCommitCallback(boost::bind(&LLFloaterModelWizard::setState, this, REVIEW));
	getChild<LLButton>("upload_btn")->setCommitCallback(boost::bind(&LLFloaterModelWizard::setState, this, UPLOAD));
}
...
void LLFloaterModelWizard::updateButtons()
{
	...
	for(size_t i=0; i<LL_ARRAY_SIZE(stateNames); ++i)
	{
		...
		else if ((int)i <= mLastEnabledState)
		{
			button->setEnabled(TRUE);
...
{
	LLFloaterModelPreview* floater_preview = LLFloaterModelPreview::show();
	if (!floater_preview)
	{
...
void LLFloaterModelWizard::loadModel()
{
	 ...
	 filepicker->run(boost::bind(&LLFloaterModelWizard::loadModel_continued, this, filepicker, 3));
}

void LLFloaterModelWizard::loadModel_continued(AIFilePicker* filepicker, int lod)
{
...
void LLFloaterModelWizard::executePhysicsStage(std::string stage_name)
{
	if (sInstance)
	{
		...
		{
			for (S32 i = 0; i < (S32)sInstance->mModelPreview->mModel[LLModel::LOD_PHYSICS].size(); ++i)
			{
